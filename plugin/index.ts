...
export const visualizer = (
  opts:
    ...
    | ((outputOptions: NormalizedOutputOptions) => PluginVisualizerOptions) = {}
): Plugin => {
  return {
    name: "visualizer",
    ...
    async generateBundle(
      ...
      const title = opts.title ?? "Rollup Bundle Analyzer";
      ...
      for (const [bundleId, bundle] of Object.entries(outputBundle)) {
        ...
        if (tree.groups.length === 0) {
          ...
          const leaf: any = {
            ...
            isAsset: true,
            uid: bundleUid,
            gzipSize: bundleSizes.gzipLength,
            parsedSize: bundleSizes.brotliLength,
            statSize: bundleSizes.renderedLength,
          };
          roots.push(leaf);
        } else {
          roots.push(tree);
